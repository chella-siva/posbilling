...
class TransactionUtil extends Util
{
     ...
     */
      ...
      public function getReceiptDetails1($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type , $paymentid)
    {
        $il = $invoice_layout;
        ...
        if ($transaction_type == 'sell' && $transaction->status == 'final') {
            ...
            $output['savedvalue_lable'] = $il->savedvalue_lable;
             ...
             $output['show_signature'] = $il->show_signature;
             $output['signature_image'] = $il->show_signature != 0 && ! empty($il->signature_image) && file_exists(public_path('uploads/signature_image/'.$il->signature_image)) ? asset('uploads/signature_image/'.$il->signature_image) : false;
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        //Paid & Amount due, only if final
        if ($transaction_type == 'sell' && ($transaction->status == 'final' || $transaction->status == 'draft')) {
            $paid_amount = $this->getTotalPaid($transaction->id);
            $due = $transaction->final_total - $paid_amount;
    ...
    protected function _receiptDetailsSellLines($lines, $il, $business_details)
    {
        ...
        foreach ($lines as $line) {
            ...
            $line_array = [
                ...
                'sell_price_inc_tax' => $product_variation->sell_price_inc_tax,
