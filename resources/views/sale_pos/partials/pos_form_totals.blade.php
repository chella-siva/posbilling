<div class="row pos_form_totals">
	<div class="col-md-12">
		<table class="table table-condensed mb-0">
			<tr>
				<td><b class="tw-text-base md:tw-text-sm tw-font-bold">@lang('sale.item'):</b>&nbsp;
					<span class="total_quantity tw-text-base md:tw-text-sm tw-font-semibold">0</span></td>
				<td>
					<b class="tw-text-base md:tw-text-sm tw-font-bold">@lang('sale.total'):</b> &nbsp;
					<span class="price_total tw-text-base md:tw-text-sm tw-font-semibold">0</span>
				</td>
			</tr>
			<tr>
					...
					<td @if(!Gate::check('disable_discount') || auth()->user()->can('superadmin') || auth()->user()->can('admin')) class="" @else class="hide" @endif>
						<b class="tw-text-base md:tw-text-sm tw-font-bold">
							...
							@if($is_discount_enabled)
								...
								<span class="tw-text-base md:tw-text-sm tw-font-semibold" id="total_discount">0</span>
							@endif
				...
				<td class="@if($pos_settings['disable_order_tax'] != 0) hide @endif">
					<span class="tw-text-base md:tw-text-sm tw-font-semibold">
						...
						<span class="tw-text-base md:tw-text-sm tw-font-semibold" id="order_tax">
							@if(empty($edit))
								0
							@else
				...
				<td>
					<span class="tw-text-base md:tw-text-sm tw-font-semibold">
						...
						<b class="tw-text-base md:tw-text-sm tw-font-bold">@lang('sale.shipping')(+): @show_tooltip(__('tooltip.shipping'))</b> 
				...
				@if(in_array('types_of_service', $enabled_modules))
					<td class="col-sm-3 col-xs-6 d-inline-table">
						<b class="tw-text-base md:tw-text-sm tw-font-bold">@lang('lang_v1.packing_charge')(+):</b>
						<i class="fas fa-edit cursor-pointer service_modal_btn"></i> 
						<span  class="tw-text-base md:tw-text-sm tw-font-semibold" id="packing_charge_text">
							0
						</span>
					</td>
				@endif
				...
				<td>
					<b class="tw-text-base md:tw-text-sm tw-font-bold" id="round_off">@lang('lang_v1.round_off'):</b> <span id="round_off_text">0</span>								
