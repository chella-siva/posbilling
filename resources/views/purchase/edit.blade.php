...
<script>
  const serverSerials = @json($serialData ?? []);
  const purchaselineid = {{ $purchase_variation_id }};
</script>
      ...
      __page_leave_confirmation('#add_purchase_form');
    	...
    	$(document).on('click', '.open-serial-modal', function () {
               ...
               let oldSerials = [];
                ...
                if (serverSerials[purchaselineid]) {
                  try {
                    oldSerials = JSON.parse(serverSerials[purchaselineid]);
                  } catch (e) {
                    console.error('Failed to parse serials:', e);
                    oldSerials = [];
                  }
                }
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (typeof serverSerials !== 'undefined' && Object.keys(serverSerials).length > 0) {
        localStorage.setItem(serialStorageKey, JSON.stringify(serverSerials));
    }

    // Inject hidden fields into the form
    for (const [variationId, serials] of Object.entries(serverSerials)) {
        let container = document.getElementById('add_purchase_form');
        serials.forEach((serial, index) => {
            let input = document.createElement('input');
            input.type = 'hidden';
            input.name = `purchases[${variationId}][serials][]`;
            input.value = serial;
            input.classList.add(`serial-hidden-${variationId}`);
            container.appendChild(input);
        });

        // Also update the quantity field if needed
        let qtyInput = document.querySelector(`input[data-variation-id="${variationId}"][name$="[quantity]"]`);
        if (qtyInput) {
